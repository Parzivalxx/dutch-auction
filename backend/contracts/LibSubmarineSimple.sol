...
contract LibSubmarineSimple is ProvethVerifier {
    ...
    */
    ...
    function getSubmarineId(
        ...
        bytes calldata _embeddedDAppData,
        bytes32 _witness,
        uint256 _gasPrice,
        uint256 _gasLimit
   ...
   /**
    ...
    function reveal(
        ...
        bytes32 commitBlockHash = blockhash(uint256(_commitTxBlockNumber));
        require(
            commitBlockHash != bytes32(0),
            "Commit Block is too old to retrieve block hash or does not exist"
        );

        require(
            block.number - uint256(_commitTxBlockNumber) > commitPeriodLength,
        ...
        address submarine = ecrecover(
            ...
            vee,
            keccak256(abi.encodePacked(submarineId, bytes32(1))),
            keccak256(abi.encodePacked(submarineId, bytes32(0)))
        );
