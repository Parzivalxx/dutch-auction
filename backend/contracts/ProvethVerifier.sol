...
contract ProvethVerifier {
    ...
    function decodeUnsignedTx(bytes calldata rlpUnsignedTx) internal view returns (UnsignedTransaction memory t) {
    ...
    function decodeSignedTx(bytes calldata rlpSignedTx) internal view returns (SignedTransaction memory t) {
    ...
    function decodeNibbles(bytes calldata compact, uint skipNibbles) internal pure returns (bytes memory nibbles) {
    ...
    function merklePatriciaCompactDecode(bytes calldata compact) internal pure returns (bytes memory nibbles) {
        require(compact.length > 0);
    ...
    function isPrefix(bytes calldata prefix, bytes calldata full) internal pure returns (bool) {
    ...
    function sharedPrefixLength(uint xsOffset, bytes calldata xs, bytes calldata ys) internal pure returns (uint) {
        uint i;
    ...
    function decodeProofBlob(bytes calldata proofBlob) internal view returns (Proof memory proof) {
